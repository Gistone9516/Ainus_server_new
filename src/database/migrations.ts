...
export async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    // 1-1. 작업 카테고리 테이블 (Task Categories)
    await createTaskCategoriesTable();
    await createTaskBenchmarkMappingTable();
...
//-- ============================================
//-- SECTION 1-1: 작업 카테고리 (Task Categories)
//-- ============================================

/**
 * task_categories 테이블 생성
 * 25개 표준 작업 카테고리 (WRITING, CODING, IMAGE_GEN 등)
 */
async function createTaskCategoriesTable(): Promise<void> {
  const sql = `
  ...
  `;
  await executeQuery(sql);
  logger.info('Table "task_categories" created');
}

/**
 * task_benchmark_mapping 테이블 생성
 * 작업 카테고리별 벤치마크 매핑 (주요 70% + 보조 30%)
 */
async function createTaskBenchmarkMappingTable(): Promise<void> {
  const sql = `
  ...
  `;
  await executeQuery(sql);
  logger.info('Table "task_benchmark_mapping" created');
}

/**
 * task_categories 초기 데이터 삽입 (25개 표준 작업 카테고리)
 */
async function seedTaskCategories(): Promise<void> {
  try {
    // 이미 데이터가 있는지 확인
    const checkSql = 'SELECT COUNT(*) as count FROM task_categories';
    const result = await executeQuery(checkSql) as any[];

    if (result[0].count > 0) {
      logger.info('Task categories already seeded, skipping...');
      return;
    }
    ...
    await executeModify(insertCategoriesSql);
    logger.info('25 task categories seeded successfully');

    // 벤치마크 매핑 삽입
    const insertMappingsSql = `
    ...
    await executeModify(insertMappingsSql);
    logger.info('Task benchmark mappings seeded successfully');

  } catch (error) {
    logger.error('Failed to seed task categories', error);
    throw error;
  }
}
